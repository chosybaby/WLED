...
uint16_t mode_wipe_in(void) {
  ...
  if(SEGENV.call == 0) {
    SEGENV.step = 1;
    SEGMENT.setPixelColor(6, BLUE);
    delta = strip.now;
  }
  if(SEGENV.step == 1 && strip.now >= delta + 1000) {
    SEGENV.step = 2;
    SEGMENT.setPixelColor(6, RED);
  }
    ...
    case WIPE:
      {
        for(uint16_t i=0; i<SEGLEN; i++) {
          ...
          //SEGMENT.setPixelColor(i, color_blend(BLACK, SEGCOLOR(0), transition));
      ...
      break;
    case ON: /*if(stateChanged)*/ SEGMENT.fill(SEGCOLOR(0)); break;
  }
...
class WipeInUsermod : public Usermod {
...
const char WipeInUsermod::_name[]    PROGMEM = "WipeInUsermod";
const char WipeInUsermod::_enabled[] PROGMEM = "enabled";
